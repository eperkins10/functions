...
function greeting(name) {
  console.log(`Hello ${String.apply(name)}`)
}
...
function theEliminator(contestants, loser) {
  for (i = 0; i < contestants.length; i++) {
    if (contestants.includes(loser)) {
      contestants = contestants.splice(loser, 1)
      return contestants
    } else {
      return contestants
    }
...
console.log(emailCheck("ethanperkinspro@gmail.com"))
...
function ribbetGold(budget) {
  let chocolateFrog = Math.floor(budget / 3)
  return chocolateFrog
}
totalFrogs = ribbetGold(4)
